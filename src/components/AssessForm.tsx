  ...
  return (
    <div style={{ display: 'grid', gap: 8 }}>
      <div style={{ fontSize: 12, color: '#6b7280' }}>
        画像内をドラッグして範囲選択 → 「IMEI画像に割当」 or 「シリアル画像に割当」
      </div>
      <canvas
        ref={canvasRef}
  ...
  async function handlePaste(e: React.ClipboardEvent<HTMLDivElement>) {
    ...
    for (const it of items) {
      if (it.type.startsWith('image/')) {
        ...
        setImgBase64(base64)
        setMessage('画像貼り付け完了。OCR実行または切り抜きできます。')
        e.preventDefault()
        return
      }
    }
    ...
    <div style={{ display: 'grid', gap: 16, padding: 16, maxWidth: 980, margin: '0 auto', background: '#f6f7fb' }}>
      ...
      <div style={section}>
        ...
        <div style={row2}>
          ...
          <input style={box as any} value={customer.name} onChange={(e) => setCustomer({ ...customer, name: e.target.value })} />
        </div>
        <div style={row2}>
          <div style={label}>フリガナ</div>
          <input style={box as any} value={customer.kana} onChange={(e) => setCustomer({ ...customer, kana: e.target.value })} />
        </div>
        <div style={row2}>
          <div style={label}>ご住所</div>
          <input style={box as any} value={customer.address} onChange={(e) => setCustomer({ ...customer, address: e.target.value })} />
        </div>
        <div style={row4}>
          <div style={label}>電話番号</div>
          <input style={box as any} value={customer.phone} onChange={(e) => setCustomer({ ...customer, phone: e.target.value })} />

          <div style={label}>生年月日</div>
          <input style={box as any} type="date" value={customer.birth} onChange={(e) => setCustomer({ ...customer, birth: e.target.value })} />
        </div>
      </div>

      {/* 3uTools 画像貼付け & OCR & クロップ */}
      <div style={section}>
        <div style={row2}>
        ...
        {imgBase64 && (
          <div style={{ marginTop: 12 }}>
            <Cropper
              image={imgBase64}
              onCropToImei={(url) => setImeiCrop(url)}
              onCropToSerial={(url) => setSerialCrop(url)}
            />
          </div>
        )}
      </div>
      ...
      <div style={section}>
        <div style={row4}>
          ...
          <input style={box as any} value={device.model_name} onChange={(e) => setDevice({ ...device, model_name: e.target.value })} />
          <div style={label}>容量</div>
          <input style={box as any} value={device.capacity} onChange={(e) => setDevice({ ...device, capacity: e.target.value })} />
        </div>
        ...
        <div style={row4}>
          ...
          <input style={box as any} value={device.color} onChange={(e) => setDevice({ ...device, color: e.target.value })} />
        ...
        {/* IMEI（上に切り抜きプレビュー） */}
        {imeiCrop && (
          <div style={{ margin: '6px 0 2px 160px' }}>
            <img src={imeiCrop} alt="imei-crop" style={{ maxHeight: 60, border: '1px solid #e5e7eb', borderRadius: 6 }} />
          </div>
        )}
        <div style={row4}>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            <button
              onClick={() => copyAndOpen(device.imei, 'https://snowyskies.jp/imeiChecking/')}
              style={{ padding: '6px 10px', borderRadius: 8, border: '1px solid #ddd' }}
              title="IMEIをコピーしてサイトを開く"
            >
              利用制限確認
            </button>
          </div>
        ...
        {serialCrop && (
          <div style={{ margin: '6px 0 2px 160px' }}>
            <img src={serialCrop} alt="serial-crop" style={{ maxHeight: 60, border: '1px solid #e5e7eb', borderRadius: 6 }} />
          </div>
        )}
        <div style={row4}>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            <button
              onClick={() => copyAndOpen(device.serial, 'https://checkcoverage.apple.com/?locale=ja_JP')}
              style={{ padding: '6px 10px', borderRadius: 8, border: '1px solid #ddd' }}
              title="シリアルをコピーしてAppleの保証確認を開く"
            >
              保証状態確認
            </button>
          </div>
      ...
      {/* 価格ブロック（amemoba検索 + 競合価格） */}
      <div style={section}>
        <div style={row4}>
          <div style={label}>MAX買取価格</div>
          <input style={box as any} placeholder="例）51000" value={maxPrice} onChange={(e) => setMaxPrice(e.target.value as any)} />
          <div style={label}>減額（合計）</div>
          <input style={box as any} placeholder="例）3000" value={discount} onChange={(e) => setDiscount(e.target.value as any)} />
        </div>
        <div style={{ height: 8 }} />
        <div style={row2}>
          <div style={label}>本日査定金額</div>
          <input style={box as any} value={todayPrice} readOnly />
        </div>
          ...
          <div style={{ border: '1px dashed #d1d5db', borderRadius: 10, padding: 8 }}>
            ...
            {!geoError && geoResult && (
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 6, fontSize: 14 }}>
                ...
                <div>
                  <span style={{ color: '#6b7280' }}>未使用：</span>
                  {geoResult.unusedText
                    ? `¥${geoResult.unusedText}`
                    : (geoResult.unused ? `¥${geoResult.unused.toLocaleString()}` : '-')}
                </div>
                <div>
                  <span style={{ color: '#6b7280' }}>中古：</span>
                  {geoResult.usedText
                    ? `¥${geoResult.usedText}`
                    : (geoResult.used ? `¥${geoResult.used.toLocaleString()}` : '-')}
                </div>
      ...
      <div style={section}>
        ...
        <div style={row2}>
          ...
          <textarea
            style={{ ...box, height: 88, resize: 'vertical' } as any}
